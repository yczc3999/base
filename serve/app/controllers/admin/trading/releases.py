...
async def list_releases(request: Request, session: AsyncSession = Depends(get_admin_read_db),
...
async def get_release(release_id: int, session: AsyncSession = Depends(get_admin_read_db),
                      auth: AuthInfo = Depends(require_all_perms("v2:release:view"))):

...
async def list_components(request: Request, session: AsyncSession = Depends(get_admin_read_db),
...
async def get_component(component_id: int, session: AsyncSession = Depends(get_admin_read_db),
                        auth: AuthInfo = Depends(require_all_perms("v2:components:view"))):

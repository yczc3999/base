...
async def list_decisions(request: Request, session: AsyncSession = Depends(get_db),
...
async def get_decision(decision_id: int, session: AsyncSession = Depends(get_db),
                       auth: AuthInfo = Depends(require_all_perms("v2:decisions:view"))):

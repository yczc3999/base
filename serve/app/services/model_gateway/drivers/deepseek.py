"""DeepSeek direct/relay wire mapping（WP-02 Checkpoint B）。
...
class DeepSeekDriver(ModelDriver):
    driver_name = "deepseek"
    provider = "deepseek"
        ...
        payload = redact(
            {
                ...
                "max_tokens": model_request.max_tokens or 2048,
            }
        )
        headers = {"Content-Type": "application/json"}
        if model_request.seed is not None:
            payload["seed"] = model_request.seed
        try:
            ...
            raise ProviderError(
                "deepseek_transport_failed", retriable=True, detail="transport"
            ) from exc
        if status >= 500:
            raise ProviderError("deepseek_5xx", status_code=status, retriable=True, detail=str(status))
        if status == 429:
            raise ProviderError("deepseek_rate_limited", status_code=429, retriable=True, detail="429")
        if status >= 400:
            raise ProviderError("deepseek_4xx", status_code=status, retriable=False, detail=str(status))
        try:
            ...
            raise ProviderError(
                "deepseek_response_malformed", retriable=False, detail="parse"
            ) from exc

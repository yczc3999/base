"""Packy OpenAI-compatible 无搜索 relay mapping（WP-02 Checkpoint B）。

只允许显式的无搜索 fallback attempt（任务 §2.10）；transport 注入以便离线 golden fixture 测试。
"""
...
class PackyDriver(ModelDriver):
    ...
    async def request(self, model_request: ModelRequest) -> ModelResponse:
        ...
        try:
            ...
            raise ProviderError("packy_transport_failed", retriable=True, detail="transport") from exc
        if status >= 500:
            raise ProviderError("packy_5xx", status_code=status, retriable=True, detail=str(status))
        if status == 429:
            raise ProviderError("packy_rate_limited", status_code=429, retriable=True, detail="429")
        if status >= 400:
            raise ProviderError("packy_4xx", status_code=status, retriable=False, detail=str(status))
        try:
            ...
            raise ProviderError("packy_response_malformed", retriable=False, detail="parse") from exc

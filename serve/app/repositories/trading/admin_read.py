...
class AdminReadRepository:
    ...
    async def dashboard_projection(self, session: AsyncSession, table: str) -> list[dict]:
        ...
        result = await session.execute(text(f"SELECT * FROM trading.{table} ORDER BY id"))
        return _rows(result)
    ...
    async def _keyset_page(
        ...
        cursor_st: Any | None, cursor_id: str | None, limit: int,
        extra_where: str = "", params: dict | None = None,
    ) -> tuple[list[dict], bool]:
        op = ">" if direction == "asc" else "<"
        order = "ASC" if direction == "asc" else "DESC"
        base_params: dict = {"st": cursor_st,
                            "cid": int(cursor_id) if cursor_id is not None else None}
        if params:
            base_params.update(params)
        if cursor_st is None or cursor_id is None:
            # 首屏：无 keyset 游标 → 取最前 limit+1 行（无 WHERE）
            where = ""
        else:
            where = f"( {sort_col}, id ) {op} ( :st, :cid )"
            if extra_where:
    ...
    async def _id_page(self, session, *, table: str, columns: str, direction: str,
                       cursor_id: str | None, limit: int,
                       extra_where: str = "", params: dict | None = None,
                       order_col: str = "id") -> tuple[list[dict], bool]:
        op = ">" if direction == "asc" else "<"
        order = "ASC" if direction == "asc" else "DESC"
        base_params: dict = {"cid": int(cursor_id) if cursor_id is not None else None}
        if params:
            base_params.update(params)
        where = f"{order_col} {op} :cid"
        if extra_where:
            where += f" AND {extra_where}"
        result = await session.execute(
            text(
                f"SELECT {columns} FROM trading.{table} "
                f"WHERE {where} ORDER BY {order_col} {order} LIMIT :lim"
            ),
            {**base_params, "lim": limit + 1},
        )
        rows = _rows(result)
        return rows[:limit], len(rows) > limit
    ...
    async def list_markets(self, session, *, cursor_st, cursor_id, direction, limit,
        ...
        return await self._keyset_page(
            ...
            cursor_st=cursor_st, cursor_id=cursor_id, limit=limit,
    ...
    async def market_chain(self, session, market_id: int) -> dict:
        ...
        specs = await session.execute(
            text(
                ...
                " cspec.kc_resolution_states, cspec.token_count, cspec.status, "
                " cspec.content_hash, pf.outcome_index, pf.function_ir, "
                " pf.algorithm_hash "
        ...
        current = await session.execute(
            text("SELECT * FROM trading.pm_market_current WHERE market_id = :mid"),
            {"mid": market_id},
        )
    ...
    _COMPONENT_COLS = (
        "id::text AS id, component_key, cost_budget, description, created_at::text AS created_at"
    )

    async def list_components(self, session, *, cursor_id, direction, limit) -> tuple[list[dict], bool]:
        return await self._id_page(
            session, table="forecast_components", columns=self._COMPONENT_COLS,
            direction=direction, cursor_id=cursor_id, limit=limit,
        )

    async def get_component(self, session, component_id: int) -> dict | None:
        result = await session.execute(
            text(
                "SELECT id::text AS id, component_key, cost_budget, description, "
    ...
    async def component_chain(self, session, component_id: int) -> dict:
        ...
        versions = await session.execute(
            text(
                "SELECT id::text AS id, version_no, cost_budget, status, content_hash, "
        ...
        members = await session.execute(
            text(
                ...
                " fccs.component_version_id::text AS component_version_id, fccs.h_c "
    ...
    async def list_episodes(self, session, *, cursor_st, cursor_id, direction, limit,
    ...
    async def episode_chain(self, session, episode_id: int) -> dict:
        ...
        prior = await session.execute(
            text(
                "SELECT id::text AS id, version_no, content, content_hash, status, "
                " created_at::text AS created_at "
        ...
        submissions = await session.execute(
            text(
                "SELECT id::text AS id, submission_key, status, q, u, "
                " algorithm_hash, committed_at::text AS committed_at, "
                " created_at::text AS created_at "
    ...
    async def episode_timeline(self, session, *, episode_id: int, cursor_st, cursor_id,
                               direction, limit) -> tuple[list[dict], bool]:
        result = await session.execute(
            text(
                "SELECT 'submission' AS kind, fs.id::text AS id, "
                " fs.created_at::text AS created_at, fs.status AS state "
                "FROM trading.forecast_submissions fs WHERE fs.episode_id = :eid "
                "UNION ALL "
                "SELECT 'gate' AS kind, gd.id::text AS id, "
                " gd.committed_at::text AS created_at, gd.gate AS state "
                "FROM trading.gate_decisions gd "
                "WHERE gd.target_kind = 'episode' AND gd.target_id = :eid "
                "UNION ALL "
                "SELECT 'info_snapshot' AS kind, inf.id::text AS id, "
                " inf.created_at::text AS created_at, inf.gate AS state "
        ...
        if cursor_id is not None:
            ...
            for i, row in enumerate(rows):
                if row["id"] == cursor_id and row["created_at"] == cursor_st:
                    cut = i + 1
                    break
            rows = rows[cut:]
    ...
    async def list_decisions(self, session, *, cursor_st, cursor_id, direction, limit,
    ...
    async def decision_chain(self, session, decision_id: int) -> dict:
        ...
        plans = await session.execute(
            text(
                "SELECT id::text AS id, plan_version, entry_range, hold_to_resolution, "
                " thesis_hash, invalidation, edge_close_threshold::text AS edge_close_threshold, "
    ...
    async def list_intents(self, session, *, cursor_st, cursor_id, direction, limit,
                           status=None) -> tuple[list[dict], bool]:
        extra, params = [], {}
        if status is not None:
    ...
    async def list_orders(self, session, *, cursor_st, cursor_id, direction, limit,
                          status=None) -> tuple[list[dict], bool]:
        extra, params = [], {}
        if status is not None:
    ...
    async def list_positions(self, session, *, cursor_id, direction, limit) -> tuple[list[dict], bool]:
        return await self._id_page(
    ...
    async def list_ledger(self, session, *, cursor_st, cursor_id, direction, limit,
    ...
    async def list_model_routes(self, session, *, cursor_id, direction, limit) -> tuple[list[dict], bool]:
        return await self._id_page(
            session, table="model_role_bindings", columns=(
                "id::text AS id, strategy_version_id::text AS strategy_version_id, "
                "role, provider, route, model_ref, network_policy, capability, "
    ...
    _AI_COLS = (
        ...
        "ai.input_tokens, ai.output_tokens, ai.cost_estimated::text AS cost_estimated, "
        "ai.created_at::text AS created_at"
    )

    async def list_ai(self, session, *, cursor_st, cursor_id, direction, limit,
    ...
    async def get_ai(self, session, *, occurred_at: str, ai_id: int) -> dict | None:
        ...
        result = await session.execute(
            text(
                ...
                " retriable, input_tokens, cache_tokens, output_tokens, "
                " reasoning_tokens, tool_count, search_count, "
    ...
    async def ai_chain(self, session, *, occurred_at: str, ai_id: int) -> dict:
        ...
        tools = await session.execute(
            text(
                ...
                " source_urls, result_artifact_ref, cost::text AS cost, "
    ...
    async def list_costs(self, session, *, cursor_st, cursor_id, direction, limit,
    ...
    async def list_config(self, session, *, cursor_st, cursor_id, direction, limit,
                          status=None) -> tuple[list[dict], bool]:
        extra, params = [], {}
        if status is not None:
    ...
    async def get_config(self, session, config_id: int) -> dict | None:
        result = await session.execute(
            text(
                "SELECT id::text AS id, config_key, version_no, content, "
    ...
    async def list_releases(self, session, *, cursor_st, cursor_id, direction, limit,
                            status=None) -> tuple[list[dict], bool]:
        extra, params = [], {}
        if status is not None:
    ...
    async def list_labels(self, session, *, cursor_st, cursor_id, direction, limit,
    ...
    async def list_metrics(self, session, *, cursor_st, cursor_id, direction, limit,
                           status=None) -> tuple[list[dict], bool]:
        extra, params = [], {}
        if status is not None:
        ...
        return await self._keyset_page(
            session, table="metric_runs mr", columns=(
                ...
                "mr.n_market, mr.n_episode, mr.n_eff::text AS n_eff, "
    ...
    async def list_promotions(self, session, *, cursor_st, cursor_id, direction, limit,
                              status=None) -> tuple[list[dict], bool]:
        extra, params = [], {}
        if status is not None:
    ...
    async def list_replay(self, session, *, cursor_st, cursor_id, direction, limit) -> tuple[list[dict], bool]:
        return await self._keyset_page(
            session, table="replay_runs rr", columns=(
                "rr.id::text AS id, rr.run_key, rr.replay_kind, rr.manifest_hash, "
                "rr.code_hash, rr.seed, rr.input_artifact_hash, rr.output_artifact_hash, "
    ...
    async def get_replay(self, session, replay_id: int) -> dict | None:
        result = await session.execute(
            text(
                ...
                " seed, input_artifact_hash, output_artifact_hash, "
                " created_at::text AS created_at "
    ...
    async def list_alerts(self, session, *, cursor_st, cursor_id, direction, limit,
    ...
    async def list_workflows(self, session, *, aggregate_type: str, aggregate_id: str,
                             cursor_st, cursor_id, direction, limit) -> tuple[list[dict], bool]:
    ...
    async def integrity_chain(self, session, *, aggregate_type: str, aggregate_id: str,
        ...
        calls = await session.execute(
            text(
                ...
                " request_hash, response_hash, status_code, latency_ms, error_reason, "
                " created_at::text AS created_at "
    ...
    async def artifact_metadata(self, session, content_hash: str) -> dict | None:
        result = await session.execute(
            text(
                ...
                " ao.original_size AS content_length, ao.stored_size AS stored_size, "
